...
export default class Ethdebugger {
  ...
  sourceLocationFromInstructionIndex(
    ...
    this.callTree.sourceLocationTracker.getSourceLocationFromInstructionIndex(
      address,
      instIndex,
      this.solidityProxy.contracts,
  ...
  extractStateAt(step: any, callback: Function): void {
    this.solidityProxy.extractStateVariablesAt(step, function(
      error: Error,
      stateVars: any
    ) {
  ...
  debug(tx: any): boolean {
    ...
    this.tx = tx;
    let self = this;
    this.traceManager.resolveTrace(tx, function(error: Error, result: any) {
      if (result) {
