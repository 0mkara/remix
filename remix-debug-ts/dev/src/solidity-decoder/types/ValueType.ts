...
export default class ValueType {
  ...
  async decodeFromStack(
    ...
    if (stackDepth >= stack.length) {
      ...
      value = this.decodeValue(
        stack[stack.length - 1 - stackDepth].replace("0x", "")
      );
    }
    return { value: value, type: this.typeName };
  }
  /** * decode the type with the @arg offset location from the memory * * @param {Int} stackDepth - position of the type in the stack * @return {String} - memory * @return {Object} - decoded value */ decodeFromMemory(
    offset: any,
    memory: any
  ): any {
